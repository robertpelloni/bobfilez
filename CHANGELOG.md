...
### Added
- **CLI**: `--count` flag for scan and duplicates commands — outputs only the count instead of full file/group listings.
- **CLI**: `--prune` flag documented in help text (removes stale DB entries for files no longer on disk).

## [2.3.8] - 2026-03-09

### Added
- **CLI**: `ignore` command with `add`, `remove`, `list` subcommands for managing ignore rules (`.gitignore`-style pattern exclusion).
- **CLI**: JSON output support for `ignore` command via `--format=json`.

### Fixed
- **Build**: Eliminated all 3 MSVC C4996 deprecation warnings by replacing `ctime()` with `ctime_s()` and `localtime()` with `localtime_s()` on Windows (with POSIX fallback via `#ifdef`).

## [2.3.7] - 2026-03-09

### Added
- **CLI**: `--verbose` flag for progress reporting during long operations (scan count, duplicate groups, verification phase progress to stderr).
- **CLI**: Parallel strong hash verification for `--mode=safe|paranoid` when using `--threads=<N>`. Uses flattened task list with per-thread `IHasher` instances and main-thread regrouping.

## [2.3.6] - 2026-03-09

### Added
- **CLI**: `--time` flag to display elapsed wall-clock time after any command (ms or seconds auto-format to stderr).
- **CLI**: `apply_filters()` now universally applied to all 8 scan-based commands: `scan`, `duplicates`, `hash`, `stats`, `metadata`, `ocr`, `classify`, `organize`.
